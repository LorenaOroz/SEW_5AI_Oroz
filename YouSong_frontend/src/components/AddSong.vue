    ...
    <div class="bg-primary p-8 rounded-lg shadow-lg w-full max-w-md relative">
      ...
      <form @submit.prevent="submitForm">
        ...
        <div class="mb-4">
          ...
          <input id="genre" v-model="song.genre" class="mt-1 block w-full px-3 py-2 border rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" placeholder="Pop, Country, etc.">
        </div>
        <div class="mb-4">
          <label for="length" class="block text-sm font-medium text-black">Length (in minutes)</label>
          <input type="text" id="length" v-model="song.length" class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" placeholder="Enter length in minutes">
        </div>
        <div class="mb-4">
          <label for="file" class="block text-sm font-medium text-black">Upload File</label>
          <input type="file" id="file" @change="handleFileUpload" class="mt-1 block w-full px-3 py-2 border rounded-md shadow-sm" />
        </div>
        <div>
...
export default {
  ...
  methods: {
    handleFileUpload(event) {
      ...
      reader.onload = (e) => {
        this.song.fileData = e.target.result;
      };
    ...
    submitForm() {
      SongService.createSong(this.song)
          .then((response) => {
            this.$emit('song-added', response.data);
            this.closeForm();
          })
          .catch((error) => {
